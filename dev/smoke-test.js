...
ok(
  !/XMLHttpRequest|sendBeacon|new WebSocket|EventSource|import\s*\(/.test(script),
  "no XHR / beacon / websocket / dynamic import in app code",
);
...
ok(
  (swSrc.match(/\bfetch\s*\(/g) || []).length === 1,
  "sw.js holds the project's single fetch() (same-origin)",
);
...
function whenReady(cb, tries = 0) {
  ...
  if (tries > 300) {
    console.log("\n❌ app did not boot (no #screen-welcome.active)");
    process.exit(1);
  }
...
whenReady(() => {
  try {
    ...
    ok(
      /No activities yet/.test($("#home-dynamic").textContent),
      "recent-activity shows an empty state",
    );
    ...
    ok(
      !/estimated/.test($("#home-dynamic .label").textContent),
      "hero switches from estimate to tracked figure",
    );
    ...
    ok(
      $$("#insights-dynamic table.sr-only").length > 0,
      "charts include sr-only data tables (a11y)",
    );
    ...
    ok(
      $("#sheet").classList.contains("open") &&
        d.querySelector('#catpick .opt[data-c="food"]').getAttribute("aria-pressed") === "true",
      "switching category swaps fields in place (sheet stays open, tab marked pressed)",
    );
    setVal($("#d-sub"), "redmeat");
    setVal($("#d-qty"), "2");
    ...
    ok(
      store().logs.find((l) => l.sub === "redmeat" && l.kg === 7),
      "computes 2 × red-meat = 7 kg",
    );
    ...
    const f = $$("#sheet button,#sheet input,#sheet select,#sheet [tabindex]").filter(
      (x) => !x.disabled,
    );
    ...
    setVal($("#set-size"), "xl");
    $("#set-hc").checked = true;
    $("#set-hc").dispatchEvent(new w.Event("change", { bubbles: true }));
    ok(
      /ts-xl/.test(d.documentElement.className) && /hc/.test(d.documentElement.className),
      "text-size and high-contrast apply",
    );
    ...
    click(d.querySelector('#nav [data-go="home"]'));
    click($("#log-btn"));
    click($("#quicks .quick")); // quick-log → toast → live()
    return setTimeout(() => {
      try {
        ok(
          $("#live").textContent.trim().length > 0,
          "an action announces via the aria-live region (screen reader)",
        );
