...
whenReady(() => {
  try {
    ...
    ok(w.kgFor("food", "veg", 1) < w.kgFor("food", "redmeat", 1), "veg meal emits less than red meat");
    ...
    const b = w.baselineFrom({ km: 20, mode: "car", diet: "mixed", cook: "lpg", shop: "some", bill: 1500, house: 3 });
    ok(b.total > 0, "produces a positive annual total");
    ok(["travel", "food", "home", "shopping"].every(k => typeof b.byCat[k] === "number"), "breaks down into all four categories");
    ok(b.house >= 1, "household size is at least 1");
    const more = w.baselineFrom({ km: 80, mode: "car", diet: "mixed", cook: "lpg", shop: "some", bill: 1500, house: 3 });
    ...
    try { loaded = w.load(); } catch { loadThrew = true; }
    ok(!loadThrew && loaded && typeof loaded === "object", "load() recovers from corrupt localStorage (returns defaults)");
    const realSet = w.localStorage.setItem.bind(w.localStorage);
    w.localStorage.setItem = () => { throw new Error("QuotaExceeded"); };
    let saveThrew = false;
    try { w.save(); } catch { saveThrew = true; }
    w.localStorage.setItem = realSet;
    ok(!saveThrew, "save() swallows a storage-quota failure instead of crashing");

    ok(errors.length === 0, "no uncaught JS errors while loading the app" + (errors.length ? ":\n" + errors.join("\n") : ""));
