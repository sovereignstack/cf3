...
const full = (u) => {
  const x = new URL(typeof u === "string" ? u : u.url, SCOPE);
...
class MockCache {
  ...
  async addAll(urls) {
    for (const u of urls) this.m.set(full(u), { kind: "precached", url: full(u) });
  }
  async put(req, res) {
    this.m.set(full(req), res);
  }
  async match(req, opts) {
    if (this.m.has(full(req))) return this.m.get(full(req));
    if (opts && opts.ignoreSearch) {
      const p = pathOf(req);
      for (const [k, v] of this.m) if (k.split("?")[0] === p) return v;
    }
    return undefined;
  }
}
function makeCaches() {
  ...
  return {
    store,
    async open(n) {
      if (!store.has(n)) store.set(n, new MockCache());
      return store.get(n);
    },
    async keys() {
      return [...store.keys()];
...
async function fireFetch(h, req) {
  let called = false,
    resp;
  h.fetch({
    request: req,
  ...
  return { called, value: called ? await resp : undefined };
}
...
(async () => {
  try {
    ...
    {
      const { handlers, caches } = loadSW(async () => {
    ...
    section("activate — drop stale caches");
    {
      ...
      ok(
        keys.some((k) => /v2/.test(k)),
        "keeps the current versioned cache",
      );
    }
    ...
    {
      let networkCalls = 0;
      const { handlers, caches } = loadSW(async (r) => {
        networkCalls++;
        return { kind: "network", url: r.url };
      });
      await fireInstall(handlers);

      const hit = await fireFetch(handlers, request(SCOPE + "index.html"));
      ok(
        hit.called && hit.value.kind === "precached" && networkCalls === 0,
        "serves a precached asset from cache (no network)",
      );

      const q = await fireFetch(handlers, request(SCOPE + "?source=pwa"));
      ok(
        q.called && q.value.kind === "precached",
        "ignoreSearch: '/?source=pwa' resolves to the cached shell",
      );

      const before = networkCalls;
      const miss = await fireFetch(
        handlers,
        request(SCOPE + "not-precached.json", { mode: "cors" }),
      );
      ok(
        miss.value && miss.value.kind === "network",
        "uncached request falls through to the network",
      );
    ...
    section("fetch — offline navigation falls back to the shell");
    {
      ...
      ok(
        nav.value && nav.value.kind === "precached" && nav.value.url === "/index.html",
